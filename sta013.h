...
// global variables
//uint16_t Sta013UpdateIndex;

// prototypes
void sta013HWReset(void);
uint8_t  sta013ReadReg(uint8_t reg);
void sta013WriteReg(uint8_t reg, uint8_t data);
void sta013DownloadUpdate(void);
uint8_t  sta013Init(void);
...
void sta013GetMP3Info(uint16_t *bitrate, uint8_t *sampFreq, uint8_t *mode);
uint16_t sta013GetAverageBitrate(void);

void sta013SetVolume(uint8_t volume, int8_t balance);
void sta013SetTone(int8_t bassEnh, uint16_t bassFreq, int8_t trebleEnh, uint16_t trebleFreq);


uint8_t  sta013Demand(void);
