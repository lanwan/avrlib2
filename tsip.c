		...
		for(i=1; i<(rxBuffer->datalength)-1; i++)
		{
			...
			if((bufferGetAtIndex(rxBuffer,i) == DLE) && (bufferGetAtIndex(rxBuffer,i+1) == ETX))
			{
				...
				if(debug)
				{
					...
					{
						rprintfuint8_t(TsipPacket[k]);
						rprintfChar(' ');
					}
					//rprintfuint8_t(bufferGetFromFront(rxBuffer)); rprintfChar(' ');
					//rprintfuint8_t(bufferGetFromFront(rxBuffer)); rprintfChar(' ');
...
void tsipProcessGPSTIME(uint8_t* packet)
{
	...
	GpsInfo.WeekNum	= ((uint16_t)packet[5]<<8)|((uint16_t)packet[6]);
