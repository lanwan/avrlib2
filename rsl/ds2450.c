...
uint8_t ds2450Result(dallas_rom_id_T* rom_id, uint8_t channel, uint16_t* result)
{
	...
	*result = 0;
	//*result = (((uint16_t)data[1] << 8) | data[0]) >> (16 - resolution);
...
{
	//const uint8_t bytes_to_read = 10;		// read 10bytes = 2/ch*4ch + CRC
	...
	for(i=0;i<8;i+=2)
	{
		...
		result[error] = 0;
		//result[error] = (((uint16_t)data[i+1] << 8) | data[i]) >> (16 - resolution[i]);
...
void ds2450Print(uint16_t result, uint8_t range)
{
	...
	rprintf(".");
	rprintfNum(10, 4, FALSE, '0', (((uint32_t)(result%vscale))*10000)/vscale );
