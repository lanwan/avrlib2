...
uint8_t ds18b20Setup(dallas_rom_id_T* rom_id, uint8_t resolution, int8_t alarm_low, int8_t alarm_high)
{
	uint8_t error;
	...
	// verify the data
	if ((int8_t)dallasReadByte() != alarm_high)	// 0x02, alarm high
		return DALLAS_VERIFY_ERROR;
	if ((int8_t)dallasReadByte() != alarm_low)		// 0x03, alarm low
		return DALLAS_VERIFY_ERROR;
...
uint8_t ds18b20Start(dallas_rom_id_T* rom_id)
{
	uint8_t error;
...
/*------ DallasTempGetResult ------*/
uint8_t ds18b20Result(dallas_rom_id_T* rom_id, uint16_t *result)
{
	uint8_t error;
...
uint8_t ds18b20StartAndResult(dallas_rom_id_T* rom_id, uint16_t *result)
{
	uint8_t error;
...
void ds18b20Print(uint16_t result, uint8_t resolution)
{
	// print raw value
	rprintfProgStrM(" 0x");
	rprintfu16(result);
	...
	rprintf(".");
	rprintfNum(10, 4, FALSE, '0', (10000*((uint32_t)(result&0x000F)))/16 );
	rprintfProgStrM(" C");
}
