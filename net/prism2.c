...
//	debugPrintHexTable(34*2, (uint8_t*)TxHeader);
//	rprintfCRLF();
//	debugPrintHexTable(len, &packet[14]);
	...
	prism2WriteBAP0(txfid, 68, (uint16_t*)&packet[14], (len+1)>>1);
...
void prism2SetupTxHeader(uint16_t* header)
{
	uint16_t i;
...
unsigned int nicPoll(unsigned int maxlen, unsigned char* packet)
{
	...
	prism2ReadBAP0(rxfid, 46, (uint16_t*)&packet[0], 6);
	// skip length, snap, and ctrl fields
	// begin data copy again at type field
	prism2ReadBAP0(rxfid, 46+12+8, (uint16_t*)&packet[12], packetLength-6);
	// ACK the receive event to finish up
	prism2Write16(PRISM2_REG_EVACK, PRISM2_EVENT_RX);
...
void prism2GetMacAddress(uint8_t* macaddr)
{
	uint16_t buffer[5];
...
void prism2SetSSID(uint8_t* ssid)
{
	uint16_t buffer[12];
...
void prism2SetWEPKey(uint8_t* wepkey)
{
	uint16_t buffer[9];
...
uint8_t prism2Command(uint16_t cmd, uint16_t param0)
{
	uint16_t result;
...
uint8_t prism2ReadRID(uint16_t id, uint16_t offset, uint16_t* data, uint16_t len)
{
	prism2Command(PRISM2_CMD_ACCESS_RD, id);
	return prism2ReadBAP0(id, offset, data, len);
}

uint8_t prism2WriteRID(uint16_t id, uint16_t offset, uint16_t* data, uint16_t len)
{
	uint8_t result;
...
uint8_t prism2ReadBAP0(uint16_t id, uint16_t offset, uint16_t* data, uint16_t len)
{
...
uint8_t prism2WriteBAP0(uint16_t id, uint16_t offset, uint16_t* data, uint16_t len)
{
	// wait for BAP to be ready
	//rprintf("PRISM_BAP: Wait1\r\n");
	while( prism2Read16(PRISM2_REG_BAP0OFFSET) & PRISM2_BAPOFFSET_BUSY);
...
void prism2CardRegDump(void)
{
	...
	rprintf("COR : "); rprintfuint8_t(prism2ReadMem(0x3E0+PCMCIA_ATTR_COR)); rprintfCRLF();
	rprintf("CSR : "); rprintfuint8_t(prism2ReadMem(0x3E0+PCMCIA_ATTR_CSR)); rprintfCRLF();
	rprintf("PRR : "); rprintfuint8_t(prism2ReadMem(0x3E0+PCMCIA_ATTR_PRR)); rprintfCRLF();
	rprintf("SCR : "); rprintfuint8_t(prism2ReadMem(0x3E0+PCMCIA_ATTR_SCR)); rprintfCRLF();
}
