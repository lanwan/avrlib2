...
void ataSetDrivePowerMode(uint8_t DriveNo, uint8_t mode, uint8_t timeout)
{
	// select drive
...
void ataPrintSector( uint8_t *Buffer)
{
	uint8_t i;
	uint16_t j;
	uint8_t *buf;
...
void ataReadDataBuffer(uint8_t *Buffer, uint16_t numBytes)
{
	unsigned int i;
...
void ataWriteDataBuffer(uint8_t *Buffer, uint16_t numBytes)
{
...
uint8_t ataStatusWait(uint8_t mask, uint8_t waitStatus)
{
                            	...
                            	unsigned char *Buffer)
{
	...
	else
	{
		...
		sect = (uint8_t) (lba % ataDriveInfo.sectors)+1;
		lba = lba / ataDriveInfo.sectors;
		head = (uint8_t) (lba % ataDriveInfo.heads);
		...
		cyl = (uint16_t) lba;
			...
			rprintfuint8_t(head); rprintfProgStrM(":");
			rprintfuint8_t(sect); rprintfCRLF();
		#endif
...
void ataDriveSelect(uint8_t DriveNo)
{
...
uint8_t ataReadByte(uint8_t reg)
{
...
void ataWriteByte(uint8_t reg, uint8_t data)
{
	//sbi(MCUCR, SRW);			// enable RAM waitstate
...
{ 
	...
	rprintfProgStrM("R0: DATALOW  = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_DATAL	));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R1: ERROR    = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_ERROR	));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R2: SECT CNT = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_SECCOUNT));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R3: SECT NUM = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_STARTSEC));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R4: CYL LOW  = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_CYLLO	));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R5: CYL HIGH = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_CYLHI	));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R6: HEAD/DEV = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_HDDEVSEL));		rprintfProgStrM(" \r\n");
	rprintfProgStrM("R7: CMD/STA  = 0x");	rprintfuint8_t(ataReadByte(ATA_REG_CMDSTATUS1));	rprintfProgStrM("\r\n");
} 
